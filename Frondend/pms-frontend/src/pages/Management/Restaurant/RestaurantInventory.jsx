    ...
    <div className="space-y-4">
      <Card className="p-5 space-y-3">
        <div>
          ...
          <p className="text-sm text-gray-600">SKU, stock, and unit cost.</p>
        </div>
        <div className="grid md:grid-cols-5 gap-3">
          <Input placeholder="SKU" value={inventoryForm.sku} onChange={(e) => setInventoryForm((f) => ({ ...f, sku: e.target.value }))} />
          <Input placeholder="Description" value={inventoryForm.desc} onChange={(e) => setInventoryForm((f) => ({ ...f, desc: e.target.value }))} />
          <Input placeholder="Stock" type="number" value={inventoryForm.stock} onChange={(e) => setInventoryForm((f) => ({ ...f, stock: e.target.value }))} />
          <Input placeholder="Minimum" type="number" value={inventoryForm.minimo} onChange={(e) => setInventoryForm((f) => ({ ...f, minimo: e.target.value }))} />
          <Input placeholder="Cost" type="number" value={inventoryForm.costo} onChange={(e) => setInventoryForm((f) => ({ ...f, costo: e.target.value }))} />
          <select
        ...
        <div className="flex justify-end">
          <Button variant="secondary" onClick={addInventory} disabled={saving}> {saving ? "Saving..." : "Add item"} </Button>
        </div>
        {inventory.length > 0 && (
            ...
            {inventory.map((i) => (
              <div key={i.id} className="border rounded-lg px-3 py-2 flex items-center justify-between text-sm">
                <div>
                  ...
                  <div className="text-xs text-gray-600">Stock: {i.stock || 0} {i.unidad || ""} | Min: {i.minimo || 0} {i.unidad || ""} | Cost: {i.costo || 0}</div>
                </div>
                <Button size="sm" variant="outline" onClick={() => removeInventory(i.id)}>Remove</Button>
              </div>
            ))}
          </div>
        )}
