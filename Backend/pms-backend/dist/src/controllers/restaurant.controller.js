...
export async function createSection(req, res) {
    ...
    if (!id || !name)
        return res.status(400).json({ message: "id y name requeridos" });
...
export async function deleteSection(req, res) {
    ...
    if (!sectionId)
        return res.status(400).json({ message: "sectionId requerido" });
...
export async function addTableToSection(req, res) {
    // @ts-ignore
    const user = req.user;
    if (!user?.hotelId)
        return res.status(401).json({ message: "No autenticado" });
    ...
    if (!sectionId || !id || !name)
        return res.status(400).json({ message: "sectionId, id y name requeridos" });
...
export async function deleteTableFromSection(req, res) {
    // @ts-ignore
    const user = req.user;
    if (!user?.hotelId)
        return res.status(401).json({ message: "No autenticado" });
    ...
    if (!sectionId || !tableId)
        return res.status(400).json({ message: "sectionId y tableId requeridos" });
    const section = await prisma.restaurantSection.findFirst({ where: { id: sectionId, hotelId: user.hotelId } });
    if (!section)
        return res.status(404).json({ message: "Seccion no encontrada" });
...
export async function listMenu(req, res) {
    ...
    if (!user?.hotelId)
        return res.status(401).json({ message: "No autenticado" });
    const sectionId = req.query.section || undefined;
...
export async function addMenuItem(req, res) {
    ...
    if (!sectionId || !name)
        return res.status(400).json({ message: "sectionId y name requeridos" });
    const section = await prisma.restaurantSection.findFirst({ where: { id: sectionId, hotelId: user.hotelId } });
    if (!section)
        return res.status(404).json({ message: "Seccion no encontrada" });
...
export async function deleteMenuItem(req, res) {
    ...
    if (!sectionId || !itemId)
        return res.status(400).json({ message: "sectionId y itemId requeridos" });
...
export async function createOrUpdateOrder(req, res) {
    ...
    if (!user?.hotelId)
        return res.status(401).json({ message: "No autenticado" });
    const { sectionId, tableId, items, note, covers } = req.body;
...
export async function getRestaurantConfig(req, res) {
    // @ts-ignore
    const user = req.user;
    if (!user)
        return res.status(401).json({ message: "No autenticado" });
    if (!user.hotelId)
        return res.status(400).json({ message: "Hotel no definido" });
...
export async function listRestaurantPrinters(req, res) {
    // @ts-ignore
    const user = req.user;
    if (!user)
        return res.status(401).json({ message: "No autenticado" });
...
export async function printRestaurantOrder(req, res) {
    // @ts-ignore
    const user = req.user;
    if (!user)
        return res.status(401).json({ message: "No autenticado" });
